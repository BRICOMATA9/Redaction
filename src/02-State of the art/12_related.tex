...
\subsection{Utility Function}
 ...
 \begin{frame}{Utility Function}{Related work}
 	\Columns{.6}{.4}{
		...
		S              & set of Slices \\
		$D_{s}$        & set of devices associated to slice s \\
		...
		$Tx_{g}$       & transmission power of GW g \\
		...
		$Tx^{gain}_{d,g}$ & power gain between a GW g and a device d \\ 
		$Rx_{d,s,g}$   & Receiver sensitivity\\
		$RTD_{d} $     & instant packet delay for device d \\
		$PDB_{d}$      & packet delay budget for device d \\\\
		$U_{d,s,g}  $  & utility for device d in slice s on GW g \\ 
		\end{tabular}
	}{
...
\Columns{.6}{.4}{
	...
	\begin{equation}
		\begin{array} { l l}
			...
			C2 : \sum _ { d \in D } i_ {d,g} \cdot tx_ { d,s,g } &\leqslant Tx^{max}_{g} , \forall g \in G , \forall s \in S \\ \\
	...
	\begin{equation}
		\begin{array} { l }
			...
			U_{HCC} = \delta _ { r } \left( \sigma _ { r } w _ { r } , \tau _ { l d } w _ { t d } \right) \quad  with \quad \delta _ { r } \in \{ 0,1 \} \\\\
			U_{MCC} = \sigma _ { r } w _ { r } + \sigma _ { l d } w _ { l d } \\\\
			U_{LCC} = \sigma _ { l d } w _ { l d } \\\\
